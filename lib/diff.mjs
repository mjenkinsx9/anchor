 ...
 * @returns {{ mode: string, range?: string, reason?: string }}
 */
...
export function parseUnifiedDiff(text) {
  ...
  for (const line of text.split('\n')) {
    // A new file section starts with an UNPREFIXED `diff --git `. Hunk body lines are
    // always prefixed (+/-/space), so a content line can never bare-start with it. Bail
    // out of hunk consumption when the next file begins — otherwise a truncated hunk
    // (declared line count > lines actually present) would swallow the next file's header.
 ...
 * `force` suppresses the flag entirely. A non-finite `maxLines`/`maxFiles` (e.g. a
 * typo'd `--max-diff-lines abc` coerced to NaN) falls back to the config-derived
 * fallback rather than silently disabling the budget (NaN comparisons are always false).
 * @param {{ files: DiffFile[] } & Record<string, any>} result
 * @param {{ maxLines: number; maxFiles: number; force?: boolean; fallbackLines?: number; fallbackFiles?: number }} budget
 */
...
function prMode(target, cwd, env) {
  ...
  const view = runCmd(
    ...
    { cwd, env, timeout: 120_000 }, // large PRs can be slow; override the 30s default
  );
  if (view.code !== 0) {
  ...
  const diff = runCmd('gh', ['pr', 'diff', target.selector], { cwd, env, timeout: 120_000 });
