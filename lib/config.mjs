...
const GITIGNORE_HEADER = '# Anchor (personal code review state)';
// The actual ignore PATTERNS — the only lines that decide whether an append is needed.
// The header comment is presentation, never a dedup key (a repo that renames/numbers
// the section, e.g. "# 17. Anchor ...", still ignores every path below).
...
export function ensureGitignore(repoDir) {
  ...
  // (block entries are already trimmed) — keeps the append idempotent.
  const lines = new Set(existing.split('\n').map((l) => l.trim()));
  // Decide solely on the real patterns. If they are all present, leave the file
  // untouched — never re-append the bare header comment just because the repo uses
  // its own header string (that was the orphan-comment churn bug).
  const missing = GITIGNORE_PATTERNS.filter((l) => !lines.has(l));
  if (missing.length === 0) return { added: false };
  // Prepend the header only when adding patterns AND it isn't already present verbatim.
  const block = lines.has(GITIGNORE_HEADER) ? missing : [GITIGNORE_HEADER, ...missing];
  const sep = existing.length && !existing.endsWith('\n') ? '\n' : '';
  appendFileSync(file, `${sep}${block.join('\n')}\n`);
  return { added: true };
}
