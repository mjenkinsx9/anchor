...
import { listLearnings, addLearning, removeLearning } from './learn.mjs';
...
const VALUED = new Set(['format', 'reason', 'max-files', 'from-diff', 'depth', 'target', 'max-diff-lines']);
...
const HANDLERS = {
  ...
  learn(positional, flags) {
    ...
    if (action === undefined || action === 'list') return emit(listLearnings(process.cwd()), flags);
    if (action === 'add') {
